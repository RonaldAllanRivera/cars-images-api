...
- **`.github/workflows/ci-cd.yml`** — a `test` job on every push and pull request (`composer validate --strict`, `check-platform-reqs`, Pint, PHPUnit, `composer audit`), and a `deploy` job with `needs: test`, so a red suite blocks the deploy. The deploy runs only from `main`, never on a pull request, and only when the repository variable `DEPLOY_ENABLED` is `true` — a switch that let the workflow be merged and exercised before any credentials existed.
  ...
  - An HTTP 200 smoke check on `/admin/login`, so a deploy that leaves the panel 500ing fails the run rather than reporting success. It retries six times over ~60s, and accepts **202** as well as 200: SiteGround's bot protection challenges the runner's datacenter IP with a 202, which is the edge answering rather than the application. A single-shot 200-only probe failed a deploy that had in fact succeeded — a false negative, the direction that teaches you to distrust the signal. No custom `User-Agent` is sent; the WAF returns 403 to unusual agent strings while curl's default is served normally.

  - **`php artisan about` on the server**, before the HTTP probe. This is the authoritative check: it boots the framework with the freshly cached config, catching the real post-upgrade failure — a stale config/route cache or a class that no longer exists — and unlike an HTTP request it cannot be challenged or masked by the edge.
